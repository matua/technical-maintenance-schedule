...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    @Override
    public Page<Schedule> findAllSortedByOptimizationIndexAndByEndExecutionDateTimeNullAndByUserEmail(Integer page, Integer pageSize, String email) {
        Pageable pageable = PageRequest.of(page, pageSize, Sort.by("optimizationIndex"));
        return scheduleRepository
                .findAllSortedByOptimizationIndexAndByEndExecutionDateTimeNullAndByUserEmail(pageable, email);
    }

    @Override
    public List<ScheduleDto> findAllByTerminalAndTaskAndByEndExecutionDateTimeNull(Terminal terminal, Task task) {
        return scheduleRepository.findAllByTerminalAndTaskAndEndExecutionDateTimeNull(terminal, task).stream().map(
                schedule -> modelMapper.map(schedule, ScheduleDto.class)).toList();
    }
    ...
    @Override
    public Page<ScheduleDto> findAllSortedByTaskPriority(Integer page, Integer pageSize) throws NotFoundException {
    ...
    public List<ScheduleDto> findByEndExecutionDateTimeNotNull() {
        return scheduleRepository.findByEndExecutionDateTimeNotNull().stream().map(
                schedule -> modelMapper.map(schedule, ScheduleDto.class)).toList();
    }
