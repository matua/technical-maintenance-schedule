...
public class TaskServiceImpl implements TaskService {
  ...
  public Task save(TaskDto taskDto) throws ValidationException, ResourceAlreadyExistsException {
    Task savedTask;
    Optional<Task> taskToSave =
        taskRepository.findByDescriptionArgsSince(getSinceArgs(taskDto.getDescription()));
    if (taskToSave.isPresent()) {
      log.info(
          "Task with such description: {} or kiosk message id: {} already " + "exists",
          taskDto.getDescription(),
          taskDto.getMessageId());
      throw new ResourceAlreadyExistsException(
          "Task with such description or message id already exists");
    } else {
      try {
  ...
  public void deleteByDescription(String taskDescription) throws NotFoundException {
    Task task =
        findByDescription(taskDescription)
