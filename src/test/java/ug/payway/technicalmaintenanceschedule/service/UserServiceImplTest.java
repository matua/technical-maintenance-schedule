...
class UserServiceImplTest {
  ...
  void save() throws ValidationException, NotFoundException, ResourceAlreadyExistsException {
    ...
    assertThat(createdUser.getFirstName())
        .isEqualTo(modelMapper.map(UserPrototype.aUser(), UserDto.class).getFirstName());
  }

  @Test
  void saveThrowsResourceAlreadyExistsException() {
    UserPrototype.aUser().setId(1L);
    when(userRepository.findByEmail(UserPrototype.aUser().getEmail()))
        .thenReturn(Optional.ofNullable(UserPrototype.aUser()));
    assertThrows(
  ...
  void update() throws ValidationException, NotFoundException {
    ...
    UserDto updatedUserDto =
        userService.update(1000L, modelMapper.map(UserPrototype.bUser(), UserDto.class));
    assertThat(updatedUserDto).isNotNull();
    assertThat(updatedUserDto.getFirstName())
        .isEqualTo(modelMapper.map(UserPrototype.bUser(), UserDto.class).getFirstName());
  }
