...
class TerminalServiceImplTest {
  ...
  @Mock PayWayApiService payWayApiService;
  ...
  void save() throws ValidationException, ResourceAlreadyExistsException {
    ...
    when(terminalRepository.findById(TerminalPrototype.aTerminal().getId()))
        .thenReturn(Optional.empty());
    when(terminalRepository.save(TerminalPrototype.aTerminal()))
        .thenReturn(TerminalPrototype.aTerminal());
  ...
  void saveThrowsResourceAlreadyExistsException() {
    TerminalPrototype.aTerminal().setId(1L);
    when(terminalRepository.findById(TaskPrototype.aTask().getId()))
        .thenReturn(Optional.ofNullable(TerminalPrototype.aTerminal()));
    assertThrows(
        ResourceAlreadyExistsException.class,
        () ->
            terminalService.save(
                modelMapper.map(TerminalPrototype.aTerminal(), TerminalDto.class)));
    verify(terminalRepository, never()).save(TerminalPrototype.aTerminal());
  }

  @Test
  void update() throws ValidationException, NotFoundException {
    when(terminalRepository.findById(any()))
        .thenReturn(Optional.ofNullable(TerminalPrototype.aTerminal()));
    when(terminalRepository.save(TerminalPrototype.bTerminal()))
        .thenReturn(TerminalPrototype.bTerminal());
    Terminal updatedTerminal =
        terminalService.update(modelMapper.map(TerminalPrototype.bTerminal(), TerminalDto.class));
  ...
  void delete() throws ValidationException, NotFoundException {
    ...
    when(terminalRepository.findById(terminalToDelete))
        .thenReturn(Optional.ofNullable(TerminalPrototype.aTerminal()));
  ...
  void findAll() throws NotFoundException {
    ...
    List<Terminal> terminals =
        List.of(TerminalPrototype.aTerminal(), TerminalPrototype.bTerminal());
  ...
  void findById() throws ValidationException, NotFoundException {
    ...
    when(terminalRepository.findById(terminalToFind))
        .thenReturn(Optional.ofNullable(TerminalPrototype.aTerminal()));
